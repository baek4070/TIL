...
public class ServerTask implements Runnable {
	...
	PrintWriter pw; // 연결된 client 로 출력
	BufferedReader br; // 연결된 client 에서 메세지 받기 
	
	String nickName; // 현재 연결된사용자의 nickName
	...
	public ServerTask(
		...
		try {
			...
			pw = new PrintWriter(bw,true); //자동으로 플러쉬 ? 
			
			InputStream is = client.getInputStream();
			InputStreamReader isr = new InputStreamReader(is);
			br = new BufferedReader(isr);
		} catch (IOException e) {
			sc.displayText("client 연결오류 : " + e.getMessage());
		}
	}
	...
	public void run() {
		...
		while(isRun) {
			try {
				//클라이언트에게 입력을 받을때까지 대기상태가됨 
				String receiveData = br.readLine();
				//그래서 receiveData가 null 이되면 멈춰줌 ! 
				if(receiveData == null) {
					isRun = false;
				}
				...
				case "0":
					...
					if(ht.containsKey(text)) {
						...
						sc.displayText(text+"-사용할수 없는 아이디 요청");
						isRun = false;
						client.close();
						break;
					}
				...
				case "1":
					...
					if(text.startsWith("/w")) {
						// /w 닉네임 메시지  
						int begin = text.indexOf(" ")+1; //문자열의 닉네임 부분의 시작인덱스
						// 두번째 매개변수 인덱스 위치부터
						// 첫번째 매개변수 글자가 어디있는 지 인덱스 번호 검색
		...
		broadCast(0,list);
		broadCast(1, nickName+"님이 나가셨습니다 남은 방인원 : "+ht.size());
		
	} //end run
	//연결된 모든 client 에 메세지 전달
